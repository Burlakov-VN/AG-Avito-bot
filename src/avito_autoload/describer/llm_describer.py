...
1. ПЕРВАЯ СТРОКА — артикул + название + УТП:
   - Формат: "[Артикул] [Название запчасти] — в наличии, быстрая доставка, выгодная цена"
   - Если артикула нет, начни с названия
   - Если цена = 0, не упоминай цену в УТП
...
3. ВНУТРЕННИЙ КОД (если поле code есть и не пустое):
   - "Внутренний код: [code]"
   - Если code пустой — НЕ пиши эту строку вообще

4. Разделитель: _____________

5. БЛОК «О ТОВАРЕ» (📌):
   - Что это за запчасть (2-3 предложения, сухо и по делу)
   - Для чего предназначена, на какую технику ставится
   - БЕЗ креативных оборотов и штампов
...
7. ИНСТРУКЦИЯ ДЛЯ ЗАКАЗА (в самом конце):
   ...
   - И ниже готовый текст для копирования с данными этой запчасти, например:
     "[Название запчасти], артикул [артикул], код [code]"
   - Включай только те данные, которые реально есть у товара
   - НЕ добавляй фразу "Хочу заказать" — только название, артикул и код

ПРАВИЛА:
- Пиши по-русски, деловой стиль, коротко и по существу
- ЗАПРЕЩЕНО: капслок, восклицательные знаки подряд, рекламные лозунги, креативные обороты
- ЗАПРЕЩЕНО: "сердце двигателя", "надежность в каждой детали", "незаменимый элемент" и подобные штампы
- НЕ пиши про компанию, доставку, оплату — добавляется автоматически
- Если каких-то данных нет (code пустой, артикул пустой) — просто пропускай эти поля, НЕ выдумывай
